...
ui <- fluidPage(
    ...
    tabsetPanel(    
            tabPanel("Community", uiOutput("community")),
            tabPanel("Debug",textOutput("debug")),
            tabPanel("Discipleship", uiOutput("discipleship")),
            tabPanel("Communal Worship", uiOutput("communal_worship")),
            tabPanel("Saraments", uiOutput("sacraments")),
            tabPanel("Evangelism", uiOutput("evangelism")),
            tabPanel("Social Action", uiOutput("social_action")),
            tabPanel("Prayer", uiOutput("prayer"))
            )
    )
...
### Selectize Reactives
  ...
  ### Activities Tibble ###### 
   ...
   output$community <- renderUI({makeInputsTab("Community", choices = choice_added(), 
                                               values = c(
   ...
   output$discipleship <- renderUI({makeInputsTab("Discipleship", choices = choice_added(),
                                                  values = c(
   ...
   output$communal_worship <- renderUI({makeInputsTab("Communal Worship", choices = choice_added(),
                                                      values = c(
   ...
   output$sacraments <- renderUI({makeInputsTab("Sacraments", choices = choice_added(),
                                                values = c(
   ...
   output$evangelism <- renderUI({makeInputsTab("Evangelism", choices = choice_added(),
                                                values = c(
   ...
   output$social_action <- renderUI({makeInputsTab("Social Action", choices = choice_added(),
                                                   values =  c(
   ...
   output$prayer <- renderUI({makeInputsTab("Prayer", choices = choice_added(),
                                            values = c(
   ...
   vars$community_feeling_5))
       ...
       message(c(vars$community_feeling_1,
                 vars$community_feeling_2,
                 vars$community_feeling_3,
                 vars$community_feeling_4,
                 vars$community_feeling_5))})
